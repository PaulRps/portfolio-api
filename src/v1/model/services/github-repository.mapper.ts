...
export class GithubRepositoryMapper {
    ...
    async toListGithubRepositoryDto(projects: any[]): Promise<GithubRepositoryDto[]> {
        if (!Array.isArray(projects)) throw new Error(`error on map githubRepository dto: ${(projects as any).message}`)
